...
export const ListproductByCategory = (keyword = '') => async (dispatch) => {
    try {
        dispatch({ type: PRODUCT_LIST_REQUEST });
        const url = `/api/products/category?keyword=${keyword}`;
...
export const createProductReview = (productId, reviews) => async (dispatch, getState) => {
    try {
        dispatch({
        ...
        await axios.post(`/api/products/${productId}/reviews`, reviews, config);
